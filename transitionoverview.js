document.addEventListener('DOMContentLoaded', ()=>{
    document.querySelectorAll('plane_proj').forEach(trigger => {
     trigger.addEventListener('click', function(){ 
       this.x = ((this.x || 0) + 1)%2; 
       if(this.x){ 
         document.getElementById('canvas_trans').forEach(target => target.classList.add('.no-scroll'));
       }
       else{ 
         document.getElementById('canvas_trans').forEach(target => target.classList.remove('.no-scroll'));
       } 
     });
    });
   });
...
if(!useNativeScroll) {
    ...
    const planeElements = document.getElementsByClassName("plane_arbeiten");
  ...
  vertexPosition.y += sin(((vertexPosition.x + 1.0) / 2.0) * 3.141592) * (sin(uPlaneDeformation / 100.0));
    ...
    function onPlaneClick(event, plane) {
        canvasclick = document.getElementById("canvas_trans"); // close button element
