...
export function projekte (smoothScroll){
   ...
   let useNativeScroll;
    ...
    curtainsProj = new Curtains({
        container: document.getElementById("canvas_proj"),
  ...
  curtainsProj.onRender(() => {
    ...
    const delta = curtainsProj.getScrollDeltas();
...
if(!useNativeScroll) {
    ...
    gsap.ticker.add(curtainsProj.render.bind(curtainsProj));
    ...
    function handlePlanes(index) {
        ...
        plane.onReady(() => {
            ...
            if(plane.userData.isFullscreen) {
                ...
                const curtainBoundingRect = curtainsProj.getBoundingRect();
    ...
    galleryState.closeButtonEl.addEventListener("click", () => {
        ...
        if(fullScreenPlane && galleryState.fullscreenThumb) {
            ...
            const allOtherPlanes = curtainsProj.planes.filter(el => el.uuid !== fullScreenPlane.uuid && el.type !== "PingPongPlane");
    ...
    function onPlaneClick(event, plane) {
        ...
        if(!galleryState.fullscreenThumb) {
            ...
            galleryState.openTween = gsap.to(animation, 2, {
                ...
                translationX: -1 * planeBoundingRect.left / curtainsProj.pixelRatio,
                translationY: -1 * planeBoundingRect.top / curtainsProj.pixelRatio,
                transition: 1,
                textureScale: 1,
                mouseX: 0,
                mouseY: 0,
                ...
                onComplete: function() {
                    ...
                    const nonClickedPlanes = curtainsProj.planes.filter(el => el.uuid !== plane.uuid && el.type !== "PingPongPlane");
    ...
    const bbox = curtainsProj.getBoundingRect();
    ...
    // our ping pong plane
    const flowMap = new PingPongPlane(curtainsProj, curtainsProj.container, flowMapParams);
    ...
    flowMap.onRender(() => {
        ...
        // update velocity
        if(!updateVelocity) {
            velocity.set(curtainsProj.lerp(velocity.x, 0, 0.5), curtainsProj.lerp(velocity.y, 0, 0.5));
        }
        updateVelocity = false;

        flowMap.uniforms.velocity.value = new Vec2(curtainsProj.lerp(velocity.x, 0, 0.1), curtainsProj.lerp(velocity.y, 0, 0.1));
    }).onAfterResize(() => {
    ...
    const shaderPass = new ShaderPass(curtainsProj, passParams);
